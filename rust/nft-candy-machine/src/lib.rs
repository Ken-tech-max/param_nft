...
anchor_lang::declare_id!("5BELdHPDp7iph7oV4EhVcgb2ChqKdnmkW6yzUTCLpEMK");
...
pub mod nft_candy_machine {
    ...
    pub fn mint_nft<'info>(ctx: Context<'_, '_, '_, 'info, MintNFT<'info>>, acc:Pubkey, count:u8) -> ProgramResult {
        let candy_machine = &mut ctx.accounts.candy_machine;
        let config = &ctx.accounts.config;
        let clock = &ctx.accounts.clock;
        msg!("23456789234567893456789{:?}", acc);
        msg!("23456789234567893456789{:?}", count);
        msg!("23456789234567893456789{:?}", &ctx.remaining_accounts);
